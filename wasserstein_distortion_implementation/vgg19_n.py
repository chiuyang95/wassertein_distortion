...
def VGG19(include_top=True, weights='normalized',
    ...
    """
    if weights not in {'imagenet', 'normalized', None}:
        raise ValueError('The `weights` argument should be either '
                         '`None` (random initialization) or `imagenet` '
                         '(pre-training on ImageNet) or `normalized` '
                         '(pre-trained on ImageNet, then normalized).')

    if weights == 'imagenet' and include_top and classes != 1000:
        raise ValueError('If using `weights` as imagenet with `include_top`'
                         ' as true, `classes` should be 1000')
    if weights == 'normalized' and include_top:
        raise ValueError('Do not include top with normalized weights.')
        ...
        if K.image_data_format() == 'channels_first':
            ...
            if K.backend() == 'tensorflow':
                warnings.warn('You are using the TensorFlow backend, yet you '
                              ...
                              'your Keras config '
                              'at ~/.keras/keras.json.')
    elif weights == 'normalized': # take the normalized weights; the weights are converted from 
